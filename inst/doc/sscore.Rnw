...
the *.CEL files later to obtain the information about outliers. {\bf The current working 
directory should have a copy of the *.CEL files whenever } \Rfunction{SScore} {\bf is called; if the working directory is 
changed, an error may result.}
...
Prior to analyzing \Robject{affybatch.example}, the data files 20A, 20B, and 10A should be copied into
the current working directory.  (These files are found in the $\backslash$inst$\backslash$doc directory of the \Rpackage{sscore}
package, and are *.CEL files corresponding to the data in the \Robject{affybatch.example} data set.)
...
## copy the files 20A, 20B, and 10A into the  current working
## directory prior to executing the next command
SScore.basic <- SScore(cel)
@ 
...
SScore.sfsdt <- SScore(cel,SF=c(22.24,25.49),SDT=c(2526.300,2590.297))
@ 
...
data(affybatch.example)
cel <- affybatch.example[,c(1,2)]
SScore.digits <- SScore(cel,digits=3)
@ 
...
SScoreBatch.sfsdt <- SScoreBatch(affybatch.example,compare=compare,
   SF=c(22.24,25.49,25.56),SDT=c(2526.300,2590.297,2751.634))
@ 
...
The \Rfunction{SScore} and \Rfunction{SScoreBatch} functions call the function \Rfunction{computeOutlier} 
to flag probes as outliers that should be excluded from the S-Score calculation. This is an internal 
function that generally will not be called or modified, but may be altered if necessary to meet specific 
needs. The \Rfunction{computeOutlier} function is called if the \verb+rm.outliers+, \verb+rm.mask+, 
or \verb+rm.extra+ parameters of \Rfunction{SScore} or \Rfunction{SScoreBatch} are set to \verb+TRUE+. As 
currently implemented, these parameters work as described in the \Rpackage{affy} documentation since they 
...
contains \verb+FALSE+. By changing the \Rfunction{computeOutlier }function, you may create additional features
not currently available, such as removing modified probes.
