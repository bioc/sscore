...
# outside of the loop, since they are constant for a given CDF or chip.
	...
	max1 <- max(pm(afbatch[,1]),mm(afbatch[,1]))*SF[1]
	max2 <- max(pm(afbatch[,2]),mm(afbatch[,2]))*SF[2]
...
# compare to S-Score calculation in J Mol Biol paper
		f.err <- (diff1-diff2)/sqrt(m.gamma*m.gamma*(diff1*diff1+diff2*diff2)+(SDT[1]*SDT[1]+SDT[2]*SDT[2])) 
...
# deviation values
	...
	fn1 <- sampleNames(afbatch)[1]
	fn2 <- sampleNames(afbatch)[2]
...
# output information on these parameters if desired by the user
	if (verbose) {
		...
		writeLines(" ")
 		writeLines(sprintf("Scaling Factor:      %8.3f     %8.3f",SF[1],SF[2]))
 		writeLines(sprintf("SDT background noise:      %8.3f     %8.3f",SDT[1],SDT[2]))
